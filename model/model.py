...
from dataset.utils import (
    ORDERED_ATTRS, _pretty_attr, get_attribute_input_fields,          
    ATTRIBUTE_CATEGORICAL_ENCODINGS, CLASS_MAPPINGS, ENCODE_AS_INT,                       
)
...
class HRGPTInterface:
    ...
    def _get_input_fields(self) -> Dict[str, Dict]:
        return get_attribute_input_fields()
    ...
    def _convert_input_to_model_format(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field, value in list(converted.items()):
            if field in ENCODE_AS_INT:
                converted[field] = ATTRIBUTE_CATEGORICAL_ENCODINGS[field][value]
    ...
    def load_test_sample(self, sample_idx: int) -> Tuple[Dict[str, Any], str]:
        ...
        for field, cfg in self.input_fields.items():
            if field in sample:
                value = sample[field]
                if cfg["type"] == "dropdown" and isinstance(value, (int, float)) and field in ENCODE_AS_INT:
                    choices: List[str] = cfg["choices"]
                    idx = int(value)
                    ui_inputs[field] = choices[idx] if 0 <= idx < len(choices) else choices[0]
                else:
                    ui_inputs[field] = value
            else:
                ui_inputs[field] = cfg.get("value", "")
